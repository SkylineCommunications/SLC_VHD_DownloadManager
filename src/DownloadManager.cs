...
public class DownloadManager
{
    ...
    public async Task<bool> DownloadAsync(string url, string outputPath, int threadCount, string? expectedHash = null, CancellationToken ct = default)
    {
        try
        {
            ...
            // Stop progress updates before any final rendering or cleanup
            try
            {
                progressMonitor.Change(Timeout.Infinite, Timeout.Infinite);
            }
            catch { /* ignore timer stop errors */ }
            progressMonitor.Dispose();

            // Final progress rendering handled explicitly below; avoid redundant update that may flicker
